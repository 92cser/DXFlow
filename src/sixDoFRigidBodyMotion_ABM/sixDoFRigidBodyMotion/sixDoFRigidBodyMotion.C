...
:
    motionState_(),
    motionState0_(),
    motionState00_(),//added for ABM method
    restraints_(),
    constraints_(),
    ...
    solver_(sixDoFSolver::New(dict.subDict("solver"), *this))
{
    ...
    // Save the old-time motion state for ABM method
    motionState00_ = motionState0_;
...
:
    ...
    motionState00_(sDoFRBM.motionState00_),
...
void Foam::sixDoFRigidBodyMotion::updateAcceleration_ABM
(
    const vector& fGlobal,
    const vector& tauGlobal
)
{
    // Calculate new accelerations. Relaxation will be done in ABM method
    a() = fGlobal/mass_;
    tau() = (Q().T() & tauGlobal);
    applyRestraints();
}
...
)
{
    if (Pstream::master())
    {
        solver_->solve(firstIter, fGlobal, tauGlobal, deltaT, deltaT0, currT);
...
void Foam::sixDoFRigidBodyMotion::status() const
{
    Info<< "6-DoF rigid body motion" << nl
        ...
        << "    Euler angles: " << quaternion(orientation()).eulerAngles(quaternion::XYZ)
        << endl;
}
